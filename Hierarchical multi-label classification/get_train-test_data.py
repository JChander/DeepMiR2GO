...
def load_data():
    ...
    gos = list()
    labels = list()
    #ngrams = list()         GOA_ensg_nonIEA_201604
    #sequences = list()      miR2GO_nonIEA_GOA_20180617
    #accessions = list()
...
def load_rep_df():
    df = pd.read_pickle('/ifs/data1/wangjiacheng/2step3/train_data/Line_miRNA_embeddings_s100_n10_512.pkl')
    return df
...
def run(*args, **kwargs):
    ...
    for i, row in df.iterrows():
        if not isinstance(row['embeddings'], np.ndarray):
            row['embeddings'] = np.zeros((64,), dtype='float32')
            missing_rep += 1
    ...
    print(len(train_df))
    train_df.to_pickle(TEST_DATA_ROOT + 'test-line_emb_s100_n10_512-' + FUNCTION + '.pkl')
